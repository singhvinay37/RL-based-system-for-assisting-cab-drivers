    ...
    def next_state_func(self, state, action, Time_matrix):
        ...
        if (pickup_loc) == 0 and (drop_loc == 0):
         ...
         ## 3. Driver wants to pickup and is at different location
            ...
            new_time, new_day = self.get_new_day_time(curr_time, curr_day, transit_time)
        ...
        new_time, new_day = self.get_new_day_time(curr_time, curr_day, total_time)
        next_state = [next_loc, new_time, new_day]
        return next_state, wait_time, transit_time, ride_time
 

    def get_new_day_time(self, time, day, duration):
        '''
        Takes the current day, time and time duration and returns new day and time based on the time duration of travel
        '''
        if time + int(duration) < 24:     ## no need to change day
            new_time = time + int(duration)
            new_day = day
        else:
            new_time = (time + int(duration)) % 24
